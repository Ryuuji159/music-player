...
export class PlaylistService {
    ...
    async register(playlistId: PlaylistId) {
        ...
        if (uniqueMedia.length !== mediaList.length) {
            this.logger.log(`Deduplicated ${mediaList.length - uniqueMedia.length} repeated media within playlist`);
        }

        const playlist = await this.prisma.playlist.upsert({
            where: { playlistId },
            update: { title: info?.title ?? "Playlist", thumbnailUrl: info?.thumbnailUrl },
            create: { playlistId, title: info?.title ?? "Playlist", thumbnailUrl: info?.thumbnailUrl }
        });
